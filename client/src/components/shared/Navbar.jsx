...
import GlobalTierBanner from "../GlobalTierBanner";
...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="bg-gradient-to-r from-green-600 to-green-700 backdrop-blur-md shadow-lg fixed w-full top-0 z-50 pb-1 rounded-b-[50px]">
        {isAuthenticated && <GlobalTierBanner />}
                ...
                <div className="relative">
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl py-2 z-10 border border-green-100">
                      <Link
                        to={`${
                          currentUser?.user?.accountType === "driver"
                            ? "/driver-dashboard"
                            : currentUser?.user?.accountType === "admin"
                            ? "/admin-dashboard"
                            : ""
                        }`}
                        className="flex flex-col items-center px-4 py-2 text-gray-700 hover:bg-green-50 transition-colors"
